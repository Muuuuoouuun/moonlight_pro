...
문의 내역은 `/dashboard/revenue/inquiries`에서 확인한다. 메일·웹훅·직접 등록은 같은 문의 원장에 저장되고, 활성 문의의 미확인 개수가 종 버튼과 오늘 화면에 표시된다. 읽음과 완료는 독립적이다. 완료·제외한 문의는 기본 알림 수에서 빠지며, 완료 후 고객의 새 회신은 문의를 다시 처리 중으로 연다. 연결된 거래 상태는 자동 변경하지 않는다.
