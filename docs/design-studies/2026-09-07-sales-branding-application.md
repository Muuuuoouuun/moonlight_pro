...
현재5개로 잘린 `items`를 다시 세지 말고 `counts`를 사용한다. 원장의160행 read 한도와 전체 count 비교로 partial 상태가 발생할 수 있다. partial에서는 `확인된 업무 기준`을 표시하고 전체 값으로 확정하지 않는다. 해당 상태 클릭 시 표시된5개에 없는 대상도 볼 수 있도록 목록 API/필터 경로를 검증해야 한다.
