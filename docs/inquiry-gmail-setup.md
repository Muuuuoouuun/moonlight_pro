...
- `apps/hub/vercel.json`에는 5분 간격이 등록돼 있다. 실제 자동 실행은 `COM_MOON_INQUIRY_AUTO_SYNC=true`일 때만 가능하다. 미설정 또는 다른 값이면 `preview / inquiry-auto-sync-disabled`를 반환한다. 배포 환경의 cron 지원 주기·요금제를 확인한 뒤 활성화한다.
