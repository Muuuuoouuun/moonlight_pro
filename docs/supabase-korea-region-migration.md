...
위 45개·7쌍은 **2026-09-20 이관 당시** 수치다. 2026-09-23에는 파일 59개·번호 중복 15쌍이며, `0044`부터 비공개 `moonlight_ops` 이력 스키마가 생겼다. 이후 `db:move-region`은 이 스키마가 있으면 public과 함께 덤프하고 이력 행 수도 검증한다.
...
- **Vercel Project·Shared 환경 변수는 2026-09-20 확인 시 0개였다.** 배포 전에 서울 프로젝트 값으로 필요한 변수를 처음부터 구성해야 한다.
