...
| eeoCRM-derived ledger | Supabase 총 119 leads 중 117 eeoCRM snapshot, 문준혁 exact-owner 16건; 증거 포함 dry-run은 16건 모두 unchanged, 태그·customer_success lane 16/16, 점수 분포 동일. live eeoCRM 연결 증거는 아님 |
...
| 계약 테스트 | 102/102 통과. readiness, Hub write guard, PMS command/idempotency, Quick Capture task/work-order receipt·duplicate·conflict·인증, task-only Today 분류·완료, native button aria forwarding, iCal fallback, MCP, webhook contract, CRM evidence apply guard, Daily Brief owner gate에 더해 `check:connections`가 Calendar enabled/configured, Gmail·Sheets disabled, OpenClaw Google MCP process-ok/provider-auth-required를 서로 다른 상태로 출력. Hub·Engine production build 2/2 통과 |
