...
const prefix='/tmp/moonlight-office-codex-pipeline';
...
const snapshot=async()=>{
  ...
  const modelConfiguration={provider:'codex-cli',model:'codex-cli-default',adapter:adapter.provenance};
...
const observed=async(input)=>{
 ...
 queue=queue.then(()=>appendFile(prefix+'.provider.jsonl',JSON.stringify({id,request,response,elapsedMs:Date.now()-started})+'\n'));
 await queue;
 return response;
};
try {
