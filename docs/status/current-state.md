...
Phase 0 is shipped. Phase 1A Durable Task Loop is implemented and has passed static, build, and production-browser verification. It is not yet declared live because this worktree has no configured Hub/Engine/Supabase environment for applying the migration and running atomic persistence smoke tests.
...
- Desktop and 390×844 browser coverage for preview, live rows, mobile editor, delayed retry-busy state, completion unlock, conflict, and failed canonical refresh.

## Verification state

- Root Node suite: 172/172 passing.
...
- Browser QA: passing with ten screenshots; one unrelated Low finding remains (`/favicon.ico` 404).
...
1. Configure Hub and Engine URLs, shared secrets, and Supabase service credentials.
2. Apply `supabase/migrations/20260713_0015_durable_task_loop.sql` through the approved migration path.
