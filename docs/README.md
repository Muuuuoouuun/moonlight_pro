...
| 회의 텍스트 검토 M0·실행 계획 M0.5 | 코드 구현·로컬 검증 / 운영 DB·배포·실사용 검증 전 | 저장된 메모의 TXT 가져오기·수동 분석·원문 근거 검토, `내가 할 일 / 함께 신경 쓸 일 / 담당 확인 필요` 분류와 날짜 역할·방법·단계 확인. 확정한 내 행동은 원문·기한·다음 행동·체크리스트를 한 트랜잭션으로 작업에 연결하고 내 작업에서 단계를 체크한다. 관련 주시는 회의 패널에만 보존되며 전역 주시 큐·알림은 아직 없다. 새 0045·0046 migration은 서울 운영 DB에 미적용. 직접 녹음·30일 원본 삭제·월 비용·CRM/Office/MCP 배분은 M1~M3. [구현·검증 기록](superpowers/plans/2026-09-23-meeting-text-review-m0.md) |
