...
4. 향후 공개 발행 표면에 필요한 `slug`, `visibility`, `published_at`, `brand` 축이 부족하다
...
향후 공개 발행 표면이 읽는 콘텐츠는 내부 운영 테이블을 직접 노출하지 말고,
...
`apps/hub`의 큐/스튜디오/퍼블리시와 향후 공개 콘텐츠 표면을 동시에 받으려면
...
향후 공개 발행 표면을 위해 아래는 공개 select view 또는 제한적 public policy가 가능하다.
