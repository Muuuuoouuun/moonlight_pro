...
1. `Politic Officer Publisher` 전용 Meta 앱과 최소 게시 범위 3개는 설정됐다. 콜백·테스터와 비즈니스 자산 연계가 미설정·미확인이므로, 정치 계정의 전용 앱 테스터 초대·OAuth와 반환 계정명을 확인한다. 초대 수락에는 Meta 약관·테스터 활동 동의가 표시된다.
