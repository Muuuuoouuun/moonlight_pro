...
- 회사 `classin.com` Chrome의 Meta Social Technologies 콘솔에는 `Classin Korea` 소유 `Classmooni`(개발 중)와 `classin-mt-ads`(라이브) 2개 앱이 보인다. 별도 `Class.Moon Publisher` 앱 생성 절차에서 Instagram 콘텐츠 관리·Threads API 이용 사례를 함께 선택할 수 있고 `Classin Korea` 포트폴리오의 비즈니스·액세스 인증 완료 표시를 확인했다. 최종 `앱 만들기`는 Meta 약관·정책 동의를 포함하므로 행동 시점 확인 대기 중이다.
