...
1. `Politic Officer Publisher` 전용 Meta 앱은 생성됐지만 게시 범위·콜백·연락처와 테스터가 미설정이다. 앱 역할은 `정상화` 관리자 1명뿐이며 정치 비즈니스 소유·관리 권한 연결은 미확인이다. `도정치` Chrome 프로필이 Codex 브라우저 목록에 나타나야 자산을 재점검할 수 있다. 최소 게시 범위 추가는 행동 시점 동의가 필요하며, 이후 테스터 초대·승인과 OAuth 동의도 별도로 확인한다. 기존 `Moonlight` 앱 초대가 정치 계정 화면에 보인 사실을 전용 앱의 승인으로 혼동하지 않는다.
...
- `도정치` 프로필은 앞선 점검에서 Meta Business Suite의 `@politic_officer` 비즈니스 자산을 볼 수 있었다. `정상화` Chrome의 Social Technologies 콘솔에 **실제 생성된** `Politic Officer Publisher` 앱(상위 앱 ID `1331254372240610`, Instagram 제품 앱 ID `3518738211624338`, Threads 제품 앱 ID `1624666812372311`)이 보인다. 앱은 미게시 상태이고 Threads API·Instagram 콘텐츠 관리 이용 사례가 있으며, 앱 역할 화면의 관리자는 `정상화` 1명이다. 기본 설정의 연락처 이메일·개인정보처리방침·앱 도메인과 두 제품의 OAuth 콜백은 비어 있다. Threads `threads_basic`은 테스트 준비 완료, 게시 권한 `threads_content_publish`와 Instagram `instagram_business_basic`·`instagram_business_content_publish`는 앱에 아직 추가되지 않았다. 생성 당시 비즈니스 선택 화면에는 `사용할 수 있는 비즈니스가 없습니다`가 표시됐고, 이후 연결 여부는 미확인이다. 별도 `도정치` Chrome 프로필이 Codex 브라우저 목록에 여전히 노출되지 않아 자산 소유·권한을 재확인하지 못했다. `dev.meta.ai/api-keys`는 AI 모델 API 콘솔로 게시 앱 콘솔과 다르다.
