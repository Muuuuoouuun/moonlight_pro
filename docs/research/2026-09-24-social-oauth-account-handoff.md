...
| YouTube 회사 `클래스인 문` | 운영자가 `junhyuk.mun@classin.com`의 채널 보기·동영상 관리 범위에 명시적으로 동의한 뒤 OAuth 완료. Hub 상태 API `connected`, 채널 `클래스인 문`(`UCNK7qVBPx7HJ0gpJw6DacrQ`), 갱신 토큰 만료 예정 2026-10-01 08:33:57 UTC 확인 | 장기 OAuth 운영 조건 검토. 업로드 API는 아직 구현되지 않음 |
...
- `도정치` 프로필은 Meta Business Suite에서 `@politic_officer` 비즈니스 자산을 볼 수 있다. 운영자는 `정상화` Chrome에서 Meta 개발자 등록을 완료했다고 알렸다. 새로 열린 `dev.meta.ai/api-keys`는 AI 모델 API 콘솔로, Instagram·Threads 게시 앱을 만드는 Social Technologies 콘솔과 다르다. 같은 Chrome에서 `developers.facebook.com`은 여전히 `Login`을 보이고 누르면 Business Suite로 돌아가며, `facebook.com`은 로그인 화면을 보인다. 따라서 이 Chrome의 Facebook 로그인 완료 전에는 정치 브랜드용 별도 Social Technologies 앱을 만들 수 없다.
- 회사 `classin.com` Chrome의 Meta Social Technologies 콘솔에는 `Classin Korea` 소유 `Classmooni`(개발 중)와 `classin-mt-ads`(라이브) 2개 앱이 보인다. 별도 `Class.Moon Publisher` 앱 생성 절차에서 Instagram 콘텐츠 관리·Threads API 이용 사례를 함께 선택할 수 있고 `Classin Korea` 포트폴리오의 비즈니스·액세스 인증 완료 표시를 확인했다. 초안 앱 연락처는 `junhyuk.mun@classin.com`이다. 운영자가 이 회사 이메일의 소멸 가능성을 제기했으므로 지속 가능한 연락처와 예비 앱 관리자 확정 전 생성 버튼을 누르지 않는다. 최종 `앱 만들기`는 Meta 약관·정책 동의도 포함한다.
- `Classin Korea` 비즈니스 포트폴리오의 `사람` 화면에는 준혁 문 외에도 전체 액세스 권한을 가진 사람이 표시되므로 회사 포트폴리오 자체가 한 이메일에만 묶인 상태는 아니다. 그러나 `앱` 자산 화면에서 기존 `Classmooni`의 전체 액세스 권한자는 `준혁 문 (You)` 1명뿐이다. 회사 이메일이 없어져도 Meta 계정 ID가 자동 삭제된다고 단정할 수는 없지만, 이메일이 유일한 로그인·복구 수단이거나 회사가 사람의 비즈니스 권한을 회수하면 앱 관리 연속성은 보장되지 않는다. [Facebook은 계정 연락처 이메일 추가·변경을 지원](https://www.facebook.com/help/ipad-app/224049364288051?locale=en_US)한다. 앱 자산에 다른 신뢰 가능한 관리자를 실제로 할당하고, 연락처를 장기 유지되는 주소로 설정해야 한다.
- 회사 `클래스인 문` YouTube의 `youtube.com/account_advanced`에는 `브랜드 계정으로 채널 이전`이 보인다. 현재는 `junhyuk.mun@classin.com` Google 계정의 개인 기본 채널이며 브랜드 계정으로 이전하지 않았다. [YouTube 공식 안내](https://support.google.com/youtube/answer/4642409?hl=en-6)는 개인 Google 계정 채널은 본인만 접근할 수 있고 브랜드 계정은 여러 소유자를 둘 수 있다고 설명한다. 회사 계정 삭제 전에 채널 소유권 이관 계획을 별도로 세워야 한다. [채널 이전 안내](https://support.google.com/youtube/answer/3056283?hl=en)는 대상 브랜드 계정을 잘못 선택하면 기존 채널이 삭제될 수 있다고 경고한다. 이번 조사에서는 이전·권한 변경을 실행하지 않았다.
