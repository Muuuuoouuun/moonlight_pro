...
- `도정치` 프로필은 앞선 점검에서 Meta Business Suite의 `@politic_officer` 비즈니스 자산을 볼 수 있었다. `정상화` Chrome은 이후 Facebook 로그인과 Social Technologies 개발자 등록을 마쳐 `내 앱` 화면이 열렸다. `Politic Officer Publisher` 앱 만들기에서 Threads API·Instagram 콘텐츠 관리 이용 사례를 선택했으나, 비즈니스 단계에는 `사용할 수 있는 비즈니스가 없습니다`가 표시된다. 앱은 생성하지 않았고 이 초안 화면을 유지했다. 현재 Codex 브라우저 연결에는 별도 `도정치` 프로필이 노출되지 않아 자산 소유·권한을 재확인하지 못했다. `dev.meta.ai/api-keys`는 AI 모델 API 콘솔로 게시 앱 콘솔과 다르다.
