...
| YouTube 정치 `도정치` | 운영자가 Politic Officer의 YouTube 계정이라고 확인. 채널 ID·OAuth 연결은 아직 검증하지 않음 | 해당 계정의 실제 채널 ID와 연결 대상 여부 확인 |
...
| `@politic_officer` Threads·Instagram | 앞선 브라우저 점검에서 두 플랫폼의 본인 로그인과 Instagram 프로페셔널 대시보드를 확인. Instagram `Moonlight-IG` 테스터 초대는 계정 화면에 `회원님이 승인함 2026년 9월 24일`로 표시됨. Threads `Moonlight`는 초대 탭에 표시됨. 당시 Meta 로그인 주체와 YouTube 계정 `도정치`의 관계는 확인되지 않음 | 전용 앱의 Threads·Instagram 초대 상태·OAuth 승인·반환 계정명 확인 |
...
> **2026-09-24 운영자 최종 정정:** `도정치`는 Politic Officer의 **YouTube 계정**이다. Instagram·Threads 또는 Meta의 별도 계정·프로필 이름으로 취급하지 않는다. 과거 브라우저에서 정치 Meta 자산이 보인 관찰은 유지하되, 그때의 Meta 로그인 주체와 `도정치` Google 계정의 관계는 확인되지 않았다. Meta 후속 작업에 `도정치`라는 이름의 Chrome 연결을 필수 조건으로 두지 않고, 실제 Instagram·Threads 로그인과 Meta 자산 권한을 확인한다.
...
- 앞선 브라우저 점검에서 Meta Business Suite의 `@politic_officer` 비즈니스 자산이 보였으나, 그때의 Meta 로그인 주체는 재확인하지 못했다. `도정치`는 운영자가 확인한 Politic Officer YouTube 계정 이름이며 이 Meta 접근 기록의 계정 식별자가 아니다. `정상화` Chrome의 Social Technologies 콘솔에 **실제 생성된** `Politic Officer Publisher` 앱(상위 앱 ID `1331254372240610`, Instagram 제품 앱 ID `3518738211624338`, Threads 제품 앱 ID `1624666812372311`)이 보인다. 앱은 미게시 상태이고 Threads API·Instagram 콘텐츠 관리 이용 사례가 있으며, 앱 역할 화면의 관리자는 `정상화` 1명이다. 기본 설정의 연락처 이메일·개인정보처리방침·앱 도메인과 두 제품의 OAuth 콜백은 비어 있다. 운영자의 2026-09-24 명시 승인 후 최소 권한 `threads_content_publish`, `instagram_business_basic`, `instagram_business_content_publish`를 각각 추가했고 세 권한 모두 **테스트 준비 완료** 표시를 확인했다. 댓글·메시지·삭제 권한은 추가하지 않았다. 생성 당시 비즈니스 선택 화면에는 `사용할 수 있는 비즈니스가 없습니다`가 표시됐고, 이후 연결 여부는 미확인이다. 앞선 Codex 브라우저 목록에 `도정치`라는 이름이 없었던 사실로 Meta 자산 소유·권한을 판단하지 않는다. `dev.meta.ai/api-keys`는 AI 모델 API 콘솔로 게시 앱 콘솔과 다르다.
