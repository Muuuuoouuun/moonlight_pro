...
1. `Politic Officer Publisher` 전용 Meta 앱은 생성됐고 최소 게시 범위 3개가 테스트 준비 완료다. 콜백·연락처와 테스터는 미설정이다. 앱 역할은 `정상화` 관리자 1명뿐이며 정치 비즈니스 소유·관리 권한 연결은 미확인이다. `도정치` Chrome 프로필이 Codex 브라우저 목록에 나타나야 자산을 재점검할 수 있다. 이후 테스터 초대·승인과 OAuth 동의는 별도로 확인한다. 기존 `Moonlight` 앱 초대가 정치 계정 화면에 보인 사실을 전용 앱의 승인으로 혼동하지 않는다.
...
- 기존 `SUPABASE_ACCESS_TOKEN`은 관리 API에서 401을 반환한다. 앞서 적용된 `0046` 마이그레이션은 로그인된 서울 프로젝트 SQL Editor와 이력 함수로 적용했다. `0047`·`0048`은 아직 미적용이며, `npm run db:check`·`npm run db:migrate`를 다시 사용하려면 관리 토큰 갱신이 필요하다.
