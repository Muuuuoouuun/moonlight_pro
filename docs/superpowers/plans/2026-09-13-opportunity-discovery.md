...
- 2026-09-17 운영자가 문의 동기화 주기를 일 1회로 확정했다. `apps/hub/vercel.json`의 `/api/cron/inquiries-sync`를 `0 21 * * *`(KST 06:00, 아침 크론 체인 앞)로 바꿔 별도 PR(브랜치 `claude/nice-hawking-8b04jt-vercel-cron-daily`)로 처리했다. Hobby 요금제의 프로젝트당 크론 2개 제한(현재 5개 등록)은 별도 확인 대상이다.
