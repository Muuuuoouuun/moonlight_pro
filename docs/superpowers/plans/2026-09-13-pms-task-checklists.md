...
## 2026-09-21 · 프로젝트 홈 계층 보정

운영자 요청: 하위 아이템 구간은 하위 프로젝트·마일스톤을 기록하고 각 항목 안에 일정과 상세 내역을 담는 공간이다.

- 홈의 상태별 개수 카드와 중복 체크리스트를 항목별 펼침 목록으로 교체한다. 각 항목의 상세 설명·다음 행동·체크리스트·개별 날짜를 내부에 표시한다.
- 기존 tasks 원장을 그대로 사용한다. `meta.item_type`은 `task` / `subproject` / `milestone`이며 기존 미분류 작업은 자동 변환하지 않는다. 여기서 하위 프로젝트는 프로젝트 안의 작업 묶음이며 독립 projects 레코드나 재귀 프로젝트 트리를 생성하지 않는다.
- `meta.checklist[].dueAt`은 선택적인 `YYYY-MM-DD` 날짜다. 하위 항목 대표 기한과 세부 날짜를 아래 일정 구간에서 모아 보여준다.
- 생성·수정은 기존 Hub → Engine 경로를 사용한다. 메타데이터 병합, 버전 비교, 재시도 중복 검사에 유형과 날짜를 포함한다. 날짜 충돌은 기존 체크리스트 충돌 선택을 따른다.
- 프로젝트 진척 산정은 기존 완료 작업 집계를 유지하며, 각 하위 항목은 세부 체크리스트 진척을 별도로 표시한다.

검증: Engine 타입 검사 통과. 전체 테스트에서 기존 생성 버튼 문구를 고정한 테스트 1건을 새 유형별 문구 계약으로 갱신했고 해당 24건 재통과. 유형·날짜 보존/검증·중복 재시도/충돌·read-back 관련 27건 통과. 브라우저에서 실제 원장 목록, 항목 펼침, 편집 탭, 날짜·메모 입력, 마일스톤 생성 시 유형·상위 프로젝트 자동 선택을 확인했다. 검증용 초안은 저장하지 않고 폐기했으며 운영 레코드는 변경하지 않았다.

## 2026-09-23 · 일정 구간 통합

프로젝트 홈의 하위 프로젝트·마일스톤과 일정은 하나의 구간으로 표시한다. 항목별 기한과 세부 일정은 각 항목 안에서 보고, 프로젝트 마감 및 모든 날짜의 모아보기는 같은 구간 안의 접힌 `일정 N` 행에서 연다. 날짜가 없으면 일정 행을 표시하지 않는다. 최근 업데이트 기록도 같은 구간의 접힌 행으로 이동한다. 저장 데이터와 진척 집계는 변경하지 않는다.
