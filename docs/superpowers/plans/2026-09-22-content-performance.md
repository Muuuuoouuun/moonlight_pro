...
- [x] Documentation/integration: README+spec 구현상태와 검증/한계 기록. 소유 파일만 commit, main 동시 변경 보존 병합, 자기 서비스/worktree 정리.
...
동시 진행된 캘린더 완료·메모 변경(e428f8e까지)을 보존해 통합했다. 통합 후 전체 1529 tests / 1518 pass / 11 skip / 0 fail, Hub production build 통과. 코드 검토와 독립 PostgreSQL 재검토 승인. 작업 브랜치는 보존하며 전용 서버와 worktree는 정리한다. 원래 작업 공간의 별도 조사 문서 수정은 건드리지 않는다. 운영 DB·배포·자동 수집 설정은 변경하지 않았다.
