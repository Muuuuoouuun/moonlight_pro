...
| # | 항목 | 처리 | 확인 |
|---|---|---|---|
| 1 | 행위자 분리 | 코드 병합(`0eba717d`·`a1d24352`·`66a4def8`). Hub는 공용 토큰 외에 `COM_MOON_AGENT_CLIENT_TOKEN_HASHES`(`actor:sha256hex` 쌍, 해시만 보관)의 클라이언트 토큰을 받고, Engine은 목록의 actor 이름을 허용한다. 형식이 틀리면 둘 다 503으로 닫힌다. `get_hub_health`가 `permissions.actorId`를 돌려준다. 처음 토큰 파일 생성은 자동 모드 안전 검사가 막았고, 운영자가 "너가 다 진행"으로 지시한 뒤 실행했다: `~/.moonlight/mcp/{claude-code,codex,claude-desktop}.env`(0600, 저장소 밖) 3개 → 해시 목록을 루트 `.env.local`에 → 세 등록을 `install <client> --mcp-env-file`로 교체(백업 `~/.moonlight/mcp/backups/`). 등록 전에 떠 있던 MCP 프로세스(열려 있던 세션, 재시작 전 Claude Desktop)는 끝날 때까지 공용 토큰, 즉 `codex`로 기록된다 | 토큰별 capabilities `actorId` 3개 일치, 각 actor로 없는 할 일 `complete_task` → `task-not-found`·저장 없음(Engine이 새 actor 허용), 가짜 토큰 401, 런처 경유 `get_hub_health` 3개 모두 자기 actor·권한 7개 |
...
| 5 | Claude Desktop 재등록 | 허브 `.env.local` 전체를 싣던 `node --env-file=…` 등록을 런처 등록으로 바꾸고, 같은 날 자기 토큰 파일(`claude-desktop.env`)을 쓰도록 다시 등록했다. 원본은 `~/.moonlight/mcp/backups/`에 백업. 새 등록은 앱을 재시작해야 읽힌다 — 이 작업을 한 대화가 그 앱 안에서 돌고 있어 에이전트가 재시작할 수 없으므로 운영자 몫이다 | `mcp:connect status --probe` 세 클라이언트 모두 정상·도구 10개·각자 env 파일 |
