...
> **2026-09-24 갱신 — §4.1 IA 재배치를 운영자 결정으로 앞당겨 채택(일부).** §7 표는 IA 재배치를 "3단계 · 2단계 실사용 뒤"로 미뤘으나, 운영자가 영업·매출 목업 4장을 검토하고 "상당히 좋은 것 같아… 진행"으로 그 게이트를 건너뛰었다. 채택한 것: `고객 연락` primary 앵커를 영업·매출 첫 탭 `오늘 연락`으로 흡수(primary 10 → 9 — 이 문서 작성 시점의 9 → 8은 그 사이 앵커가 늘어 숫자만 다르다), Leads·Accounts를 `고객`의 단계로, 개요·히트맵을 거래의 보기로 흡수, Cases는 ⌘K 전용(Q141 권장값 그대로). **채택하지 않은 것**: `기록` 렌즈 신설(§4.7), `문의`의 고객 흡수(Q142 — 목업은 문의를 별도 탭으로 둔다), `돈` 이름(→ `거래`), 옛 라우트의 `LEGACY_REDIRECTS` 전환(→ 라우트 유지 + 가장 가까운 탭 강조). 이 절(§4.1)과 §7의 3단계 행은 [`2026-09-24-revenue-four-tabs-design.md`](2026-09-24-revenue-four-tabs-design.md)가 대체한다. 나머지 절의 상태는 그대로다.
