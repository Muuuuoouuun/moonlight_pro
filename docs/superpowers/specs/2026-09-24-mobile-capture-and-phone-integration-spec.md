...
### ④ 주소록 접근 및 원터치 고객 등록 (W3C Contact Picker API)
* **표준 명세**: W3C Contact Picker API (`navigator.contacts.select(['name', 'tel'])`) 채택.
* **동작 흐름**:
  1. 갤럭시 Chrome/삼성인터넷 브라우저에서 고객 화면의 **[새 고객]** 드로어를 열면 상단에 `[주소록에서 가져오기]` 액션 표시.
  2. 탭 시 안드로이드 OS의 **순정 주소록 선택 화면** 호출.
  3. 사용자가 원하는 연락처를 체크하면 이름과 전화번호가 드로어 필드에 즉각 채워짐.
  4. 저장 시 Supabase `leads.phone` 컬럼에 번호가 영속화되어, 이후 해당 번호로 통화/문자 발생 시 CRM 자동 매칭 개시.

### ⑤ 미등록 번호 수신 시 신규 리드 후보화 (`captureUnmatched`)
* Engine `/api/intake/phone-events`는 `captureUnmatched: true` 플래그를 수신하면 기존 고객 목록과 매칭되지 않는 낯선 번호라도 버리지 않고 `isUnregistered: true` 후보로 수집.
* Hub `기록할까요` 섹션에 **"미등록 번호 010-XXXX-XXXX 통화"** 행이 생성되며, 기존 [기록 남기기] 대신 **`[+ 새 고객으로]`** 버튼 노출.
* 클릭 시 번호가 채워진 새 고객 등록 드로어로 딥링크 이동하여 1탭 리드 전환.
...
1. **CRM 관련 (전화·문자·카톡)**:
   ...
   - 등록 고객: `[기록 남기기]` (원클릭 공식 활동 영속화).
   - 미등록 고객: `[새 고객으로]` (원클릭 리드 생성 드로어 연결).
...
## 6. 갤럭시 음성 변환 (Bixby Text Call) 가능성 조사 및 분석

> **조사 과제**: 갤럭시 음성 변환 기능을 활용하여 원하는 통화를 수행할 수 있는가?

### A. 갤럭시 음성 변환 기술의 실체
1. **텍스트로 전화 받기 (Bixby Text Call)**:
   - 수신 통화 시 전화를 음성 대신 텍스트 대화창으로 응대하는 삼성 원UI 순정 기능.
   - 상대방 음성을 실시간 STT(Speech-to-Text)로 화면에 표출하고, 내가 입력한 텍스트 또는 퀵 템플릿 버튼을 TTS(Text-to-Speech)로 읽어줌.
2. **나만의 음성 만들기 (Custom Voice Creator)**:
   - 운영자가 18개 문장을 사전 녹음하여 음성 모델을 구축하면, 빅스비 기계음이 아닌 **운영자 본인의 목소리 톤**으로 상대방에게 텍스트를 읽어줌.
3. **통화 대화록(Transcript) 자동 보존**:
   - 통화 종료 후 삼성 전화 앱 통화 상세 내역에 대화 전문이 텍스트로 보존됨.

### B. 시스템 연동 가능성과 OS 보안 경계
* **완전 자동 봇 통화(Audio Injection)의 한계**:
  - Android Telecom Framework 및 삼성 Knox 보안 정책상, 서드파티 웹/앱이 **통화 중 마이크 오디오 스트림에 임의의 오디오를 실시간 주입하거나 가로채는 행위는 엄격히 금지**되어 있다(루팅 없이는 불가).
  - 삼성은 Bixby Text Call에 대한 공개 API/SDK를 외부 개발자에게 제공하지 않는다.
* **실현 가능한 권장 워크플로**:
  1. **전화 앱 빠른 답장 프리셋**: 학원 영업/상담용 핵심 응대 템플릿("미팅 중입니다. 견적 건이신가요?")을 갤럭시 전화 설정에 사전 등록.
  2. **운영자 Custom Voice 적용**: 텍스트 통화 시 본인 음성 톤으로 읽어주도록 세팅하여 수신자의 거부감 최소화.
  3. **통화 후 대화록 CRM 연동**: 통화 종료 후 전화 앱에 남은 텍스트 대화록을 MacroDroid/공유 시트로 Moonlight 고객 활동 메모로 1-Tap 전송.
