...
> 방법: 서브에이전트 5개(Engine 통합 / Hub OAuth / MCP·커맨드·스크립트 / 문서 주장 / 형제 저장소) 병렬 감사 + 라이브 Supabase 기록 읽기(`integration_connections`, `sync_runs`, `webhook_events`, `agent_runs`, `work_orders`, `error_logs`) + 핵심 주장 8건 코드 스팟체크 + Codex(gpt-5.6-sol, read-only sandbox) 독립 검토 1회(반박 4건 수용, §9). 모두 읽기 전용이며 파일·기록·설정을 바꾸지 않았다.

## 1. 한 줄 결론

연결은 많이 **만들어져** 있지만, 09-11 현재 실제로 작동하고 최근 30일 안에 쓰인 것은 **Supabase 기록과 Google Calendar OAuth 둘뿐**이다. 나머지는 네 부류로 나뉜다.
...
## 2. 실측 인벤토리 (코드 × 라이브 기록 × 소비자)

판정 기준: `작동` = 코드 완결 + 최근 기록 + 소비자 존재. `놀고 있음` = 코드 완결이나 등록/버튼/키가 없음. `반쪽` = 한쪽 방향만. `휴면` = 작동 가능하나 30일 이상 기록 없음. `죽음` = 호출 경로 없음.

| 연결 | 코드 상태 | 라이브 기록 (마지막 성공) | 소비자 | 판정 |
| --- | --- | --- | --- | --- |
| Supabase REST (`rwqe…`) | Hub·Engine·MCP 전부 사용 | 기록 자체 | 전 표면 | 작동 |
...
| Gemini Vision (명함 OCR, `GEMINI_API_KEY`) | 완결 | 별도 기록 없음 | 명함 캡처 흐름 | 작동 추정 |
...
| OpenClaw 프로젝트 webhook | **현재 코드는 provider 허용목록이 `moltbot`뿐**(`apps/engine/lib/shared-webhook.ts:6`, Hub 타깃도 `generic`·`moltbot`). `openclaw` 참조는 readiness·health·`check-connections`에만 남음 | `openclaw` connected(local) 행과 07-14 배달 기록이 기록에 잔존. 07-15 문서의 launchd 2개는 **현재 0개** | 없음 | 죽음(코드)·기록 잔존 |
...
| classin-dash MCP (`dash.classin.cloud`, http, 전역) | 읽기 전용 9도구 | 별도 Supabase 프로젝트(`xgab…`) | Claude 세션만. Moonlight 기록·문서와 무연계, `integration-inventory.md` 미기재 | 작동, 미연계 |
| claude.ai 커넥터 | Figma 인증됨(저장소 내 사용 근거 0), Notion·Google Drive 미인증 | — | — | 유휴 |
| Google 서비스 계정 / IAM | **없음**. 모든 Google 접근은 OAuth 사용자 동의. `scripts/sync-rev-ledger.mjs`만 `GOOGLE_SERVICE_ACCOUNT_*`를 요구하나 env 미설정 | — | 매출 기록 dry-run | 누락 |
...
기록 시간축 요약: calendar 09-04 → work_orders 08-04(최근 5건 전부 dismissed) → agent_runs 07-12 → eeocrm 07-07 → telegram 06-15 → github 06-05. 7월 중순 이후 Hub 사용은 캘린더·태스크·프로젝트·메모(최근 커밋 영역)에 집중되고, AI 자문·CRM 동기화·Telegram은 두 달 휴면이다.

## 3. 발견 상세

### 3.1 놀고 있는 자산 (켜기만 하면 됨)

1. **Moonlight MCP** — 17도구가 Hub 라우트 17개에 정확히 대응하고 라우트가 전부 존재한다. 어느 클라이언트에도 등록되지 않아 Claude Code·Codex 세션이 기록을 직접 읽거나 태스크를 만들 수 없다. 그래서 `/morning`·`/team`·`/inbox` 커맨드는 Supabase 테이블명을 산문으로 언급하는 방식으로 우회한다.
...
5. **classin-dash MCP** — 학원 운영 데이터 읽기 도구가 연결돼 있으나 Moonlight의 고객·프로젝트 기록과 연결 지점이 없다.

### 3.2 누락

- **Telegram 회신 leg**: 수신·처리·기록은 되는데 답장을 보낼 코드가 없다. `TELEGRAM_BOT_TOKEN`은 env 템플릿에만 있다.
...
- **Google 서비스 계정**: 없음. 무인 배치(매출 기록 싱크)는 서비스 계정이 자연스러운데 사용자 OAuth만 있다.
...
`work_orders` 26건 중 최근 기록은 전부 `dismissed`이고 `agent_runs`의 `outcome_id`는 전부 null이다. Council·Guru 자문은 생성됐지만 실행·결과로 이어진 기록이 없다. 09-09 운영 계획 §1의 진단("한 고객 → 한 판단 → 한 실행 → 한 결과를 닫는 것이 급하다")과 일치한다. Gemini·MCP·Telegram을 더 붙여도 이 루프가 닫히지 않으면 기록은 계속 조용하다. 그래서 Track C4를 통합 작업과 같은 무게로 둔다.
...
- Google 서비스 계정 도입: 매출 기록 무인 싱크가 필요해질 때. 지금은 dry-run 스크립트만 있어 미정.
...
현재 상태: 커맨드 3개가 "Codex 게이트"를 언급하지만 배선은 `/codex` 스킬 관행뿐이고, Moonlight MCP 미등록으로 Claude·Codex 어느 쪽도 기록에 닿지 못한다.
...
| 기록 쓰기 | Moonlight MCP write 도구(`create_task`·`decide_work_order`) | `COM_MOON_HUB_WRITE_SECRET` |
...
| Q2 | Telegram 회신 / OpenClaw | **둘 다 삭제** | Telegram 수신 라우트·lib·n8n fan-out·env, OpenClaw env·readiness·health·프로브 참조를 코드에서 제거. 기록 행(`webhook_events` 108건, `openclaw` connection)은 이력으로 보존 |
...
3. A1 MCP 등록 + 중복 도구 제거 + 커맨드 재배선 — Claude·Codex가 기록을 읽게 만드는 유일한 배선
...
- 라이브 기록 조회: `integration_connections` 8행, `sync_runs` 61행, `webhook_events` 131행, `agent_runs` 26행, `work_orders` 26행, `error_logs` 239행 (2026-09-11, service role, 읽기 전용).
...
- Codex 독립 검토(gpt-5.6-sol, read-only, 네트워크·기록 없음): §2 판정 24건 중 확인 ○ 11, 부분 △ 11(대부분 "기록 없이는 미검증" — 이 문서의 기록 근거로 보완), 반박 ✕ 1(OpenClaw → 수용, 판정을 "죽음"으로 정정). 계획 비판 5건 중 4건 수용: Sheets 상태 경로, Meta 발행 scope, 평문 컬럼의 보안 등급, state TTL. 추가 권장 3건 수용(4축 표, state replay 테스트, Vision apiKey).
