...
| Integration honesty and control | 14 / 14 | Health and provider status APIs distinguish live, disabled, snapshot, and fallback. Four secrets are separated. Calendar OAuth and iCal do not merge. Gmail/Sheets no longer overstate ledger presence as OAuth readiness. MCP has guarded writes and 13 tools. | No deduction in the implemented local boundary. |
