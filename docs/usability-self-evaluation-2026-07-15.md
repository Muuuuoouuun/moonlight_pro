...
This is an operator-usability score backed by live API responses, focused tests, builds, control-plane probes, and hydrated browser checks of Daily Brief, Revenue Leads, Projects, and Content at desktop plus Projects at 390×844. It is not a full product-wide pixel grade; Daily Brief and Revenue still need the same narrow-screen visual pass.

| Criterion | Score | Evidence | Deduction |
| --- | ---: | --- | --- |
| Action hierarchy and Daily Brief | 13 / 15 | Hydrated desktop runtime shows all 6 ledgers live, 7 decision signals, and live PMS/ClassIn content pulses with status counts and direct task/queue actions. ClassIn counts exclude unrelated personal brands. | Daily Brief has not yet received the same 390px pass as Projects. |
...
| Reliability and recoverability | 10 / 12 | Repository tests, Hub/Engine builds, Engine typecheck, connection checks, live PMS/content round-trips, fresh MCP 13-tool discovery plus task create/read-back/delete, and signed webhook 401/accepted/duplicate/read-back/delete all pass. OpenClaw uses Homebrew Node 24, has security critical 0, and reset its 515%-used main context without deleting the transcript. Temporary verification rows are 0. | Claude Desktop app lifecycle remains unverified; OpenClaw's corrected Telegram cron has not yet reached its first scheduled delivery. |
| **Total** | **89 / 100** |  |  |
...
3. Extend the 390px visual audit from Projects to Daily Brief and Revenue, then fix only measured overflow/hierarchy issues.
