...
@main struct PetActivityRunner {
    @MainActor static func main() async {
        do { print("PASS: Pet activity and Council draft checks (\(try await runPetActivityStoreTests()))") }
...
swiftc -parse-as-library -o "$output_dir/check" \
  ...
  "$src/Models/PetActivityStore.swift" "$src/Models/CouncilDraftStore.swift" \
  Tests/MoonlightPetPreviewTests/PetActivityStoreTests.swift "$output_dir/Runner.swift"
"$output_dir/check"
