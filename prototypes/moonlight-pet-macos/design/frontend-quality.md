...
## Reading contrast, prism and character separation — 2026-09-25

The next user screenshots showed white foreground overlapping a bright code editor. Added localized native HUD reading regions plus a neutral 28% scrim under title/control/row/calendar content. Empty space and the main clear rim stay transparent. Memo protection grows only with written/insertion lines until the viewport is full; its empty notebook area is clear. This changes the reading region's resting contrast, not the default character wash. A shared per-panel presentation source mirrors press/drag color to those regions, returning to neutral on release. Focus's opaque shield opts out of behind-window material. No text shadow or blur is added.

Eight character RGB tokens are separated in hue/lightness while keeping their existing interaction alpha 76%; Sylveon's original soft blush and 60% alpha are retained. A software sRGB compositing comparison over a neutral background showed larger pairwise color differences; that calculation is diagnostic, not a perceptual score for every desktop.

Verification: native lab grid and silver-fold backgrounds showed readable white headings/body labels with clear intervening space. The lab's explicit preview showed distinct Vaporeon blue and Sylveon pink in both the main body and reading zones, then returned to the untinted state. Korean lab text was rendered through accessibility value entry; automated Korean paste/type did not insert correctly in this run, so an IME/paste validation is not claimed. Real quick task/calendar/memo navigation and current records were inspected without editing saved data. Shared reading-tone self-check covers initial clear, preview start/end, character changes without opacity changes and accessibility override/reset; existing drag/release and persistence checks pass.

Metal output: 2,356 prism pixels, peak RGB separation 19/255, 560 refracted edge pixels in the owned calibration scene, shader center drift 0.00/255. The custom desktop layer only decorates the edge; system glass still handles desktop refraction. No claim of a tunable desktop refractive index, universal contrast ratio or measured FPS is made.
