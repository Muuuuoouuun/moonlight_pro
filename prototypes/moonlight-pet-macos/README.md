...
오른쪽 가장자리의 작은 펫을 한 번 누르면 빠른 기능 바가 열린다. 할 일과 일정에서는 창이 좁고 긴 세로형으로 바뀌며 기능 버튼도 세로로 놓인다. 다른 기능에서는 가로형으로 돌아간다. 메뉴 막대의 달 아이콘에서 선택한 캐릭터가 함께 나오는 `짧은 메시지 보기`를 열 수 있다. 펫을 우클릭하면 이브이와 진화체 아홉 캐릭터 중 하나를 선택할 수 있다. 기본값은 글레이시아이며 선택은 이 Mac에 저장된다. 어느 앱에서든 `⌃⌥M`으로 바를 열 수 있고 메뉴 막대에서도 열 수 있다. 일정·Office·Council 버튼은 브라우저에서 Hub를 연다. 기본 Hub 주소는 `http://127.0.0.1:3000`이며 바의 브라우저 화면에서 바꿀 수 있다.
