...
네 장면의 정적 배치 그림은 [four-states.svg](design/four-states.svg)에서 볼 수 있다.
...
오른쪽 가장자리 펫을 한 번 누르면 미리보기, 두 번 누르면 빠른 기능 바가 열린다. 어느 앱에서든 `⌃⌥M`으로 바를 열 수 있고 메뉴 막대의 달 아이콘에서도 열 수 있다. 일정·Office·Council 버튼은 브라우저에서 Hub를 연다. 기본 Hub 주소는 `http://127.0.0.1:3000`이며 바의 브라우저 화면에서 바꿀 수 있다.

바가 열려 있을 때 `⌘1`~`⌘6`으로 기능을 바꾸고 `⌘Return`으로 선택한 기능의 Hub 화면을 연다. `Esc`는 바를 닫는다.

할 일과 메모는 이 Mac의 목업 저장소에만 저장된다. 실제 Hub 할 일/메모와 동기화되지 않는다.
