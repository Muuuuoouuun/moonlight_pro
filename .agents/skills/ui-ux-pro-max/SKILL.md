...
This repository versions the search script at `.agents/skills/ui-ux-pro-max/scripts/search.py`. Run the examples below from the repository root. From another working directory, resolve that path against the repository root and invoke the absolute path.
