...
- `migrations/20260717_0019_routine_check_idempotency.sql`: 루틴 체크의 워크스페이스별 멱등 키 보강
- `migrations/20260717_0020_nullable_project_progress.sql`: 증거 없는 프로젝트 진행률을 `null`로 보존
- `migrations/20260718_0021_task_description.sql`: 할 일(tasks)에 자유 텍스트 `description` 컬럼 추가
- `migrations/20260719_0022_project_context_links.sql`: 프로젝트 빠른 생성용 `lead_id`/`customer_account_id` 연결 + 표준 areas 시드
- `migrations/20260719_0023_deal_hidden_at.sql`: 딜(deals)에 되돌릴 수 있는 `hidden_at` 컬럼 추가
- `apply-pending.sql`: **편의 번들** — 0003→0011 + 0019~0023을 시점순으로 묶은 단일 파일(멱등). 대시보드 SQL Editor에 한 번에 붙여넣기용. 정본은 위 개별 migration 파일.
...
4. 운영 마이그레이션 적용 (0003→0011 + 0019~0023): **간편 경로** = `apply-pending.sql` 전체를 SQL Editor에 붙여넣고 Run (멱등, 원자적). **개별 경로** = PAT(`SUPABASE_ACCESS_TOKEN=sbp_...`) 설정 후 `node scripts/apply-migrations.mjs <파일들…>`. 포함: `20260602_0003`(variant_type 5종)·`0004`·`0005`~`0009`(시트·CRM·cadence·outcomes·명함)·`0010`(페르소나 시드·inbox)·`0011`(work_orders·agent_runs)·`0019`(routine check idempotency)·`0020`(nullable project progress)·`0021`(task description)·`0022`(project context links)·`0023`(deal hidden_at).
