...
- `apply-pending.sql`: **편의 번들** — 0003→0011 + 0019~0021을 시점순으로 묶은 단일 파일(멱등). 대시보드 SQL Editor에 한 번에 붙여넣기용. 정본은 위 개별 migration 파일.
...
4. 운영 마이그레이션 적용 (0003→0011 + 0019~0021): **간편 경로** = `apply-pending.sql` 전체를 SQL Editor에 붙여넣고 Run (멱등, 원자적). **개별 경로** = PAT(`SUPABASE_ACCESS_TOKEN=sbp_...`) 설정 후 `node scripts/apply-migrations.mjs <파일들…>`. 포함: `20260602_0003`(variant_type 5종)·`0004`·`0005`~`0009`(시트·CRM·cadence·outcomes·명함)·`0010`(페르소나 시드·inbox)·`0011`(work_orders·agent_runs)·`0019`(routine check idempotency)·`0020`(nullable project progress)·`0021`(task description).
