-- Preserve current cumulative performance when restoring a Studio draft revision.
-- Apply after 20260912_0026_content_workflow.sql. No tables or columns change.
-- Publication fields retain the existing revision restore behavior.
begin;
...
begin
  ...
  if v_action = 'restore_revision' then
    ...
    -- Performance is the latest measured state, not revisioned draft content.
    -- Keep the currently locked row's value (or absence), never a snapshot's.
    v_variant_patch := jsonb_set(v_variant_patch, '{meta}',
      (coalesce(v_variant_patch->'meta', '{}'::jsonb) - 'performance')
      || case when v_variant.meta ? 'performance'
        then jsonb_build_object('performance', v_variant.meta->'performance')
        else '{}'::jsonb end);
    insert into public.content_revisions(workspace_id,content_id,variant_id,snapshot,reason)
