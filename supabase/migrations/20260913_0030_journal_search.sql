-- General-note discovery only. Existing journal reads and atomic writes stay unchanged.
-- Apply after 0027. Search text is a literal substring, never SQL or wildcard syntax.
begin;

create index if not exists journal_entries_note_cursor_idx
  on public.journal_entries(workspace_id,occurred_at desc,id desc) where entry_kind='note';
create index if not exists journal_links_context_search_idx
  on public.journal_links(workspace_id,target_type,target_id,journal_id) where link_kind='context';
create index if not exists journal_links_use_search_idx
  on public.journal_links(workspace_id,journal_id) where link_kind='use';
...
begin
  ...
  with candidates as (
    ...
    select * from candidates where (p_query='' or title_pos>0 or body_pos>0 or enhancement_pos>0)
    ...
    select id,occurred_at,jsonb_build_object(
      ...
      'revision',note_revision,'used',used,'match',case when p_query='' then null else jsonb_build_object(
        'field',case when title_pos>0 then 'title' when body_pos>0 then 'body' else 'enhancement' end,
        'text',substring(case when title_pos>0 then title when body_pos>0 then body else note_meta->>'enhancement' end
          from greatest(1,(case when title_pos>0 then title_pos when body_pos>0 then body_pos else enhancement_pos end)
