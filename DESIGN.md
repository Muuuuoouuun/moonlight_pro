...
| 2026-09-19 | Futura 텍스처를 셸과 공용 컴포넌트로 확장. (a) 사이드바는 아이콘·색점을 버리고 텍스트 전용이 되며 현재 항목은 `--surface` pill + `--fx-shadow`로 뜬다. (b) 사이드바가 **현재 섹션의 하위 목적지만** 펼친다 — 2026-07-15의 "한 단계" 규칙을 대체한다. (c) 데스크톱에서 `.hub-topbar__tabs`를 숨긴다(≤900px는 유지). (d) `Button`·`IconButton`·`Input`·`SegmentedControl`은 pill radius, `Card`는 보더 대신 그림자. (e) `dashboard/work/decisions`는 페이지 헤더가 pill 탭을 직접 그린다(`PAGE_OWNS_TABS`) | confirmed | 운영자 지시 "좌측 탭의 ui, 컴포넌트 질감 류 도" + Decisions 화면 첨부. 옛 아코디언의 부활이 아니다: 그건 workspace × function 두 축을 섞어 같은 표면을 두 번 보여준 게 문제였고, 여기는 기능 축 하나만 펼치며 caret 토글도 없다 — 펼침 여부를 운영자가 고르지 않고 현재 위치가 정하므로 "어디에 사는가"를 먼저 묻게 되지 않는다. 탑바 탭을 숨긴 이유는 사이드바가 같은 목록을 이미 그려서 한 화면에 2차 내비가 두 줄이 되기 때문. 확실성 라벨은 원안의 `COMMITTED`/`TRIAL (4W)` 대신 §5.3·§11의 한국어 직접 라벨(확정/미정)을 유지했다 — 대문자 영문은 한글 표면의 접근 가능한 이름이 아니다 |
