...
| 2026-09-23 | Work 탭 줄(Projects·Calendar·Roadmap·Decisions·Rhythm) 끝에 **OKR·KPI** 탭(`dashboard/work/goals`) 신설. 새 화면을 만들지 않고 현황의 목표·성과와 같은 `Goals` 화면(목표 → 측정 지표 → 관측, 빠른 체크)을 Work 안에서 연다 — `goalHref`에 `base`를 넘겨 탭 안의 링크·드로어·생성 딥링크가 그 탭에 머문다. 현황의 `?view=goals` 입구는 그대로 둔다(두 입구, 같은 데이터) | recommended | 운영자 지시 "OKR KPI 트래킹하는 부분, 이 부분(Rhythm 옆)에서 탭 신설". 목표 저장소(`operating_objectives`·`operating_metrics`·관측)는 이미 있어 스키마 변경이 없다. 루틴 체크를 KPI 자동 원천으로 잇는 것(`source_key`에 routine 추가)은 마이그레이션이 필요한 별도 결정으로 남긴다. 현황 입구를 없앨지는 운영자가 두 입구를 써 본 뒤 정한다 |
