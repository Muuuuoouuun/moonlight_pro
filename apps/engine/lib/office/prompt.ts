...
export function buildOfficePrompt(request:OfficeRequest, context:OfficeContext) {
 ...
 return {systemInstruction,prompt:JSON.stringify({mode:request.mode,scope:request.scope,sourceContext:context,untrustedRecentConversation:request.history,userRequest:request.message})};
}
