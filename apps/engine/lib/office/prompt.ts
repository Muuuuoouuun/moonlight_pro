...
export function buildOfficePrompt(request: OfficeRequest, context: OfficeContext) {
  ...
  const modeInstructions: Record<string, string> = {
    chat: '상담 모드(chat): 1~2턴 내에 논점을 좁힌다. 모호한 상황에서는 질문을 늘리지 않고 가장 유력한 가설 1개와 대안 1개로 선택지를 압축해 제시한다.',
    draft: '초안 모드(draft): 즉시 복사하거나 업무에 사용할 수 있는 완성형 산출물(마크다운 블록)을 생성한다. 앞뒤의 긴 잡담과 인사말은 생략한다.',
    review: '검토 모드(review): [통과 / 수정 필요 / 차단] 3단계 중 하나로 판정하고, 위험 지점의 위치와 위험 이유, 그리고 즉시 교체 가능한 대체 수정안(패치)을 세트로 제공한다.',
  ...
  const systemInstruction = [
    ...
    '운영자에게 친근하고 명료한 반말로 답한다. 호칭은 필요할 때만 대표. 울음소리·애교·과한 칭찬은 쓰지 않는다. 고객용 산출물(제안서·메시지·콘텐츠)은 고객 및 브랜드의 타깃 말투를 엄격히 따른다.',
