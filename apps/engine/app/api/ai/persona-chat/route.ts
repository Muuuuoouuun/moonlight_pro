...
function buildPrompt({
  ...
  } else if (mode === "outreach-draft") {
    lines.push(
      "【고객 연락 메시지 초안 생성 모드 (카카오톡/문자/DM)】",
      "운영자가 복사해서 바로 보낼 수 있는, 정중하고 부담 없는 맞춤 연락 메시지를 생성하라:",
      "- 원칙 1 (길이): 3~4문장 이내 (모바일 한 화면에 쏙 들어오는 간결함). 불필요한 미사여구 배제.",
      "- 원칙 2 (맥락 연결): 고객의 최근 상태, 이전 대화 내용, 또는 보낸 자료를 자연스럽게 언급.",
      "- 원칙 3 (낮은 마찰력의 질문): 상대가 '예/아니오' 또는 가볍게 답할 수 있는 캘리브레이티드 질문으로 끝맺음 (예: '~관련해서 이번 주 10분 정도 통화 괜찮으실까요?').",
      "",
      "반드시 아래 2단 구조로 답변하라:",
      "[💬 추천 메시지 초안]",
      "(실제 전송할 텍스트만 작성 - 따옴표나 군더더기 제외)",
      "",
      "[💡 대화 의도 및 다음 수]",
      "- 목표: (예: 도입 장애물 파악, 견적 검토 의사 타진)",
      "- 상대 무응답 시: (예: 3일 후 리드마그넷 공유로 재접촉)",
    );
  } else if (mode === "extract-actions") {
    lines.push(
      "【비정형 텍스트 AI 구조화 및 액션 추출 모드】",
      "날것의 메모, 회의록, 카카오톡 복사 텍스트를 분석하여 즉시 실행 가능한 행동 단위로 정리하라:",
      "1. 📌 [1줄 핵심 요약]: 전체 텍스트의 핵심 요지 한 줄.",
