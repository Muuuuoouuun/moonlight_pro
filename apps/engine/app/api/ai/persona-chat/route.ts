...
- 원칙: 모든 일에 전원을 호출하지 않고, 건별 성격에 맞는 최소 집합만 활성화합니다.
- 형식: [1. 신호 분석] -> [2. 우선순위 판정] -> [3. 활성 페르소나 지정] -> [4. 작업지시서 초안].`,
...
- 원칙: 근거 없는 구매 확신을 경계하고, 고객의 실제 저항(No)과 구매 7스타일(논리/관계/권위/직관/안정/체면/합의)에 맞춤 응대를 제시합니다.
- 형식: [1. 딜 현주소] -> [2. 고객 저항/반론 핵심] -> [3. 추천 대화 스크립트/다음 행동] -> [4. 후속 시점].`,
...
- 원칙: David Ogilvy의 사실 중심 카피와 Seth Godin의 가장 작은 유효 시장(SVM) 관점을 적용합니다. 허위 과장 표현은 배제합니다.
- 형식: [1. 핵심 앵글/가설] -> [2. 타겟 오디언스] -> [3. 훅과 개요 구조] -> [4. 발행 채널 제안].`,
...
- 형식: [1. 채널/포맷] -> [2. 슬라이드별/스레드별 헤드라인 및 본문 골격] -> [3. 시각화/제작 가이드].`,
...
- 판정: [PASS | REVISE | NEEDS_HUMAN] 중 하나를 명확히 선언합니다.
- 형식: [1. 판정 결과] -> [2. 적발된 위험/과장 표현] -> [3. 수정 권고안] -> [4. 최종 승인 조건].`,
...
function buildPrompt({
  ...
  if (mode === "critique") {
    ...
    lines.push(
      ...
      "3. 🎯 [다음 주 Council 조언: 단 1가지 가역적 실험] (다음 주에 반드시 집중할 1가지 행동과 완료 기준)",
    );
  } else if (mode === "advice") {
    lines.push(
      "【실행 조언 모드】",
      ...
      "3. 📋 [등록할 할 일/작업지시서 제안] (Moonlight 원장에 바로 등록할 제목과 내용)",
    );
  } else {
    // Default chat
    lines.push(
...
export async function POST(req: Request) {
  ...
  const prompt = buildPrompt({ personaId, mode, lens, message, context, draft });
