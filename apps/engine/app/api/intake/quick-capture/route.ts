...
import { executeQuickCaptureCommand, sanitizeTaskWriteInput } from "../../../../lib/commands/task-command.ts";
// @ts-expect-error TypeScript does not enable allowImportingTsExtensions in this workspace.
import { validateSharedWebhookRequest } from "../../../../lib/shared-webhook.ts";
export const runtime = "nodejs";
export const dynamic = "force-dynamic";

type ExecuteQuickCapture = typeof executeQuickCaptureCommand;
...
export async function handleQuickCaptureRequest(
  req: Request,
  dependencies: { execute?: ExecuteQuickCapture } = {},
) {
  ...
  const result = await (dependencies.execute ?? executeQuickCaptureCommand)({
    workspaceId,
...
export async function POST(req: Request) {
  return handleQuickCaptureRequest(req);
}
