...
test('meeting review PostgreSQL receipts, evidence, scope and task recovery', { skip: !enabled }, async () => {
  ...
  try {
    ...
    const finish = (value) => json(`select public.meeting_review_finish_v1('${workspace}','${journal}','${request}',${literal(JSON.stringify(value))}::jsonb)`);
    ...
    const result = finish({ state: 'ready', summary: '고객 검토와 후속 연락', proposals: [proposal, pendingProposal, wrongTitleProposal], model: 'test', usage: { promptTokens: 10, totalTokens: 13 } });
    assert.equal(result.status, 'saved');
    assert.equal(result.snapshot.run.state, 'ready');
    assert.equal(result.snapshot.proposals.length, 4);
