import { parseOfficeAnswer,parseOfficeContext } from '@com-moon/agent-contracts/office';
export async function callOfficeEngine(request,context,{fetcher=fetch,engineUrl=process.env.COM_MOON_ENGINE_URL,secret=process.env.COM_MOON_SHARED_WEBHOOK_SECRET}={}) {
 ...
 try {
  ...
  return {status:'generated',...answer,ownerId:request.ownerId,mode:request.mode,scope:request.scope,participants:request.participants,lens:null,simulation:data.simulation,model:typeof data.model==='string'?data.model:null,context};
