...
export function mapProjects(
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      delivery: row.meta?.delivery ? { ...row.meta.delivery, nextAction: row.next_action || "" } : null,
...
function mapTodos(rows, projectById, brandById) {
  return rows.map((row) => {
    ...
    return {
      ...
      // 오늘 3개 — 고른 날짜의 이력(meta.focus_dates, KST 'YYYY-MM-DD'). task-today.js가 오늘
      // 레인을, attention-ledger가 `focus` 버킷을, 주간 리포트가 완료율을 이걸로 센다(§6.2).
