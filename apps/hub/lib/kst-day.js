...
// 복제돼 있었다. 고객 연락 큐가 세 번째 사본을 만드는 대신 여기로 모은다. task-today의
// dateKeyInZone도 2026-09-23 통합에서 여기로 모았다(그쪽은 재수출만 남는다). 서버는 UTC로 돌기
...
// 시간대별 포맷터 캐시 — 운영자 시간대는 워크스페이스 설정에서 오므로 개수가 유한하다.
const ZONE_FORMATTERS = new Map([[TIME_ZONE, KST_DAY_FORMAT]]);

function dayFormatter(timeZone) {
  const zone = timeZone || TIME_ZONE;
  let format = ZONE_FORMATTERS.get(zone);
  if (!format) {
    format = new Intl.DateTimeFormat("en-CA", { timeZone: zone, year: "numeric", month: "2-digit", day: "2-digit" });
    ZONE_FORMATTERS.set(zone, format);
  }
  return format;
}

// ISO/Date → 'YYYY-MM-DD'(주어진 시간대). 날짜만 있는 문자열은 그대로 통과시킨다 — UTC 자정으로
// 파싱했다가 다시 KST로 포맷하면 하루가 밀릴 수 있다.
export function zonedDayKey(value, timeZone = TIME_ZONE) {
  if (!value) return "";
...
// KST 전용 얇은 래퍼 — 호출처 대부분이 시간대를 고르지 않는다.
export function kstDayKey(value) {
  return zonedDayKey(value, TIME_ZONE);
}
