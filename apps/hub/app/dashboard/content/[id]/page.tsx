  ...
  return (
    <div className="flex items-center justify-between px-6 h-14 bg-white border-b border-[rgba(0,0,0,0.07)] shrink-0">
      <div className="flex items-center gap-3">
        <h1 className="text-sm font-semibold text-[#0F0F0F] tracking-tight">콘텐츠 편집</h1>
        <span className={["text-[9px] font-semibold px-2 py-0.5 rounded-md uppercase tracking-wider border border-[rgba(0,0,0,0.07)]", statusColor[status]].join(" ")}>
          {statusLabel[status]}
        </span>
      </div>
      <div className="flex items-center gap-2">
        <span className={["text-xs transition-all duration-300", saveState === "saving" ? "text-[#9BA8B5]" : saveState === "saved" ? "text-[#16A34A]" : saveState === "error" ? "text-[#DC2626]" : "text-transparent select-none"].join(" ")}>
          {saveState === "saving" ? "저장 중..." : saveState === "saved" ? "저장됨 ✓" : saveState === "error" ? "저장 실패" : "·"}
        </span>
        {status !== "archived" && (
          <button onClick={onArchive} className="px-3 py-2 text-xs text-[#9BA8B5] hover:text-[#6B7280] hover:bg-[#F8F8F9] rounded-xl transition-all duration-100">
            보관
          </button>
        )}
        <button onClick={onSave} disabled={saveState === "saving"} className="px-3.5 py-2 text-xs font-medium text-[#0F0F0F] bg-white border border-[rgba(0,0,0,0.12)] rounded-xl hover:bg-[#F8F8F9] active:scale-[0.96] disabled:opacity-40 transition-all duration-100 shadow-sm">
          저장
        </button>
        {status !== "published" && (
          <button onClick={onPublish} disabled={saveState === "saving"} className="px-3.5 py-2 text-xs font-semibold text-white bg-[#0F0F0F] rounded-xl hover:bg-[#1A1A1A] active:scale-[0.96] disabled:opacity-40 transition-all duration-100 shadow-sm">
            발행
          </button>
        )}
      </div>
...
function Field({ label, hint, max, len, children }: { label: string; hint?: string; max?: number; len?: number; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center justify-between">
        ...
        {max !== undefined && len !== undefined && (
          <span className={["text-[10px] tabular-nums", len > max * 0.9 ? "text-[#DC2626]" : "text-[#C4C9CF]"].join(" ")}>{len}/{max}</span>
        )}
      </div>
      {children}
    ...
    <div className="flex flex-col h-full bg-white border border-[rgba(0,0,0,0.07)] rounded-2xl overflow-hidden">
      ...
      <div className="flex-1 flex flex-col gap-5 p-5 overflow-y-auto">
        <Field label="제목" max={30} len={data.title.length}>
          <input className={inputCls + " font-medium"} value={data.title} onChange={(e) => onChange({ ...data, title: e.target.value })} placeholder="핵심 내용을 담은 짧은 제목" maxLength={30} />
        </Field>
        <Field label="부제목" max={40} len={data.subtitle.length}>
          <input className={inputCls} value={data.subtitle} onChange={(e) => onChange({ ...data, subtitle: e.target.value })} placeholder="출처 · 날짜 · 카테고리" maxLength={40} />
        </Field>
        <Field label="본문" hint="줄 앞에 - 를 붙이면 항목으로 표시됩니다" max={300} len={data.body.length}>
          <textarea className={inputCls + " min-h-[180px] font-mono resize-none leading-relaxed"} value={data.body} onChange={(e) => onChange({ ...data, body: e.target.value })} placeholder={"- 첫 번째 항목\n- 두 번째 항목\n- 세 번째 항목"} maxLength={300} />
        </Field>
      </div>
    </div>
  )
  ...
  return (
    <div className="flex flex-col h-full bg-white border border-[rgba(0,0,0,0.07)] rounded-2xl overflow-hidden">
      <div className="px-5 py-3.5 border-b border-[rgba(0,0,0,0.06)] flex items-center justify-between">
        ...
        <span className="text-[10px] text-[#C4C9CF] bg-[#F8F8F9] border border-[rgba(0,0,0,0.07)] px-2 py-0.5 rounded-md">1080 × 1080</span>
      </div>
        ...
        <div className="w-full max-w-[300px] aspect-square bg-white rounded-2xl shadow-md border border-[rgba(0,0,0,0.08)] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 flex flex-col p-7 min-h-0">
            ...
            <div className="mb-auto">
              ...
              <p className="text-[11px] text-[#9BA8B5]">{data.subtitle || <span className="text-[#E5E7EB]">부제목</span>}</p>
            </div>
            <div className="w-6 h-px bg-[rgba(0,0,0,0.1)] my-4 shrink-0" />
            {data.body && <div className="text-[11px] text-[#4B5563] leading-relaxed flex-1 min-h-0 overflow-hidden" dangerouslySetInnerHTML={{ __html: bodyHtml }} />}
...
function LoadingSkeleton() {
  return (
    <div className="flex flex-col h-[calc(100vh-56px)] bg-[#F8F8F9] overflow-hidden animate-pulse">
      <div className="flex items-center justify-between px-6 h-14 bg-white border-b border-[rgba(0,0,0,0.07)]">
        <div className="h-4 w-32 bg-[#F0F0F2] rounded-lg" />
        <div className="flex gap-2">
          <div className="h-8 w-16 bg-[#F0F0F2] rounded-xl" />
          <div className="h-8 w-16 bg-[#F0F0F2] rounded-xl" />
        </div>
      </div>
...
export default function ContentEditPage() {
  ...
  useEffect(() => {
    async function load() {
      const { data: row } = await supabase
        ...
        .single()
      if (row) {
        setData({ title: row.title ?? "", subtitle: row.subtitle ?? "", body: row.body ?? "" })
  ...
  const update = useCallback(async (nextStatus?: ContentStatus) => {
    setSaveState("saving")
    const { error } = await supabase
      .from("content_items")
      .update({ title: data.title, subtitle: data.subtitle, body: data.body, ...(nextStatus ? { status: nextStatus } : {}) })
      .eq("id", id)
    if (error) {
      setSaveState("error")
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-56px)] bg-[#F8F8F9] overflow-hidden">
      <Toolbar
        ...
        status={status}
        onSave={() => update()}
        onPublish={() => update("published")}
        onArchive={() => update("archived")}
      />
