...
export function MyWork({ onNavigate }) {
  ...
  const openTaskDraft = (item) => {
    ...
    setTaskDraft({ id: item.entityId, title: item.title, status: item.status, priority: item.priority || 'medium', dueAt: item.whenAt || '', openedStatus: item.status });
  };
  ...
  // 소프트 삭제 — 퀵애드로 잘못 만든 할 일을 지운다(migration 0021, deleted_at). 완료 처리와
  ...
  const deleteTaskDetail = async () => {
    ...
    try {
      ...
      if (!res.ok || !['saved', 'preview'].includes(data.status)) {
        setNotice({ tone: 'err', label: data.error || `삭제 실패 (${data.status || res.status})` });
  ...
  return (
    <div className="hub-page" style={{ padding: 'var(--section-gap)', display: 'flex', flexDirection: 'column', gap: 'var(--gap)', height: '100%', maxWidth: 1080, margin: '0 auto', width: '100%' }}>
      ...
      <EditDrawer
        ...
        // 이 드로어의 상태 필드에 이미 '완료'가 있다 — 푸터까지 '완료'면 "편집을 저장했다"와
        // "할 일을 끝냈다"가 같은 단어가 된다. 저장 버튼은 '저장'으로 분리한다.
        saveLabel="저장"
