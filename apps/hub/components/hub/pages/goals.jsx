...
import { Button, Card, Drawer, EmptyState, Kbd, SelectField, Skeleton, TextField, TruthBadge } from '../hub-primitives';
import { goalHref, goalScope, goalSectionState, goalLinkedEntityHref } from '@/lib/goal-client';
import { useGoals, useGoalCommand } from '../use-goals';
import { GoalCommandFeedback, GoalMetricCard, GoalMetricForm, GoalMetricSummary, GoalObjectiveForm, GoalReadFeedback, goalScopeLabel } from '../goal-components';
...
export function Goals() {
  ...
  const base = goalHref(null, scope);
  ...
  const close = () => { focusAfterClose.current = { id: selectedId }; router.replace(base, { scroll: false }); };
  React.useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      ...
      const target = opener.current?.isConnected && !opener.current.closest('[role="dialog"]') ? opener.current : item || createButton.current;
  ...
  return <div className="goals-page fade-up">
    ...
    <div className="goal-fields-two"><TextField label="목표 검색" value={search} onChange={event => setSearch(event.target.value)} /><SelectField label="목표 상태" value={status} options={[{ value: 'active', label: '진행 중' }, { value: 'archived', label: '보관한 목표' }, { value: 'all', label: '전체 상태' }]} onChange={event => setStatus(event.target.value)} /></div>
    {model.status === 'partial' && <p className="goal-muted" role="status">일부 원장을 확인하지 못했습니다. 읽힌 값만 표시하며 부족한 근거로 달성을 확정하지 않습니다.</p>}
    {!['live', 'partial'].includes(model.status) || !model.objectives.length ? <GoalReadFeedback model={model} onCreate={create} /> : !filtered.length ? <EmptyState title="조건에 맞는 목표가 없습니다" action={<Button onClick={() => { setSearch(''); setStatus('all'); }}>검색·필터 지우기</Button>} /> : <ul className="goal-list">{filtered.map(objective => {
    ...
    {selectedId && !creating && <GoalDetail key={selectedId} id={selectedId} onClose={close} />}
  </div>;
}
