...
export function Studio({ workspace }) {
  ...
  const addSlide = () => {
    setSlides(s => [...s, { id: 'new-' + Date.now(), bg: 'oklch(0.3 0.02 250)', title: '', sub: '' }]);
    markDirty();
  };
        ...
        <div className="hub-studio-toolbar" style={{ padding: 'var(--pad-y) var(--section-gap)', borderBottom: '1px solid var(--line-soft)', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          <div style={{ display: 'flex', gap: 2, background: 'var(--surface-2)', border: '1px solid var(--line-soft)', borderRadius: 'var(--r-sm)', padding: 2 }}>
            {[{k:'blog',l:'Blog / Insight'},{k:'carousel',l:'Card News'}].map(m => (
              <button key={m.k} type="button" aria-pressed={mode === m.k} onClick={() => { setMode(m.k); markDirty(); }} style={{
            ...
            <div className="hub-studio-canvas scroll-y" style={{ flex: 1, padding: 'var(--section-gap)', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', gap: 24 }}>
              ...
              <Card className="hub-studio-card" style={{ width: 320 }}>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 6, marginBottom: 14, flexWrap: 'wrap' }}>
                  {['oklch(0.30 0.02 250)','oklch(0.34 0.04 245)','oklch(0.28 0.03 225)','oklch(0.36 0.025 255)','oklch(0.24 0.01 250)','oklch(0.42 0.02 250)','oklch(0.16 0.005 250)','oklch(0.92 0.005 250)'].map((c, index) => (
        ...
        <div style={{ padding: 'var(--gap)', borderTop: '1px solid var(--line-soft)' }}>
          ...
          >
            ...
            <input
              ...
              style={{ flex: 1, minWidth: 0, background: 'transparent', border: 'none', outline: 'none', color: 'var(--fg)', fontSize: 12 }}
            />
...
export function Queue({ workspace }) {
  ...
  // Soft-delete an errant draft (migration 0021 · DELETE /api/hub/content). Optimistically
