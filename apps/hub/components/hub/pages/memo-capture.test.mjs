...
test("memo-capture component includes multimodal photo and audio intake controls", () => {
  ...
  assert.match(source, /saveMemoIntakeTasks\(prepared, fetchImpl/);
  ...
  assert.match(source, /<Checkbox\s+label=/);
  assert.doesNotMatch(source, /type="checkbox"/);
  assert.match(source, /appendMemoIntake\(draftRef.current, data, file.name\)/);
  assert.match(source, /readOnly=\{memoLocked\}/);
});
