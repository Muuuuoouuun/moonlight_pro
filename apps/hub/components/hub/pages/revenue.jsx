...
// Editable detail drawer for Revenue records. Field-driven so each lane can share one
// editor while the parent owns persistence/local overlay state.
...
export function Cases() {
  ...
  const [caseEdits, setCaseEdits] = React.useState({});
  ...
  const cases = [...localCases, ...ledgerCases].map(c => (caseEdits[c.id] ? { ...c, ...caseEdits[c.id] } : c));
  ...
  return (
    <div className="hub-page" style={{ padding: 'var(--section-gap)', display: 'flex', flexDirection: 'column', gap: 'var(--gap)' }}>
      ...
      <EditDrawer
        ...
        subtitle={editingCase ? `${editingCase.id} · 운영 케이스 정보 편집` : ''}
        record={editingCase}
        fields={[
          { key: 'title', label: '제목' },
          { key: 'account', label: '계정' },
          { key: 'type', label: '대상', type: 'select', options: [{ value: 'company', label: 'Company' }, { value: 'personal', label: 'Personal' }] },
          { key: 'priority', label: '우선순위', type: 'select', options: [{ value: 'high', label: 'High' }, { value: 'med', label: 'Medium' }, { value: 'low', label: 'Low' }] },
...
function DetailPanel({ account, detail, onAction, onLog, onPinNote, onAddNote, onEdit, onNavigate }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      ...
      <div style={{ padding: 'var(--card-pad)', borderBottom: '1px solid var(--line-soft)' }}>
        ...
        <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          <Button variant="outline" size="xs" icon="edit" onClick={onEdit}>
            Edit
          </Button>
          <Button
            variant="secondary"
...
export function Accounts({ onNavigate }) {
  ...
  const [editAccountKey, setEditAccountKey] = React.useState(null);
  ...
  const ACCOUNTS = [...localAccounts, ...ledgerAccounts].map((a) => {
    const key = accountKey(a);
    return { ...a, ...accountEdits[key], _key: key, _baseName: a.name };
  });
  ...
  React.useEffect(() => {
    if (view === 'detail' && !filtered.find(a => a._key === selectedKey)) {
      setSelectedKey(filtered[0]?._key ?? null);
    }
  }, [view, filtered, selectedKey]);

  const baseDetailFor = (account) => (
    ledger.source === 'supabase'
      ? null
      : ACCOUNT_DETAIL[account._baseName] || ACCOUNT_DETAIL[account.name]
  ) || emptyDetail();

  const getDetail = (account) => details[account._key] || baseDetailFor(account);

  const pushActivity = (account, entry) => {
    setDetails(prev => {
      const cur = prev[account._key] || baseDetailFor(account);
      return {
        ...prev,
        [account._key]: { ...cur, activity: [{ at: '방금', who: 'Me', ...entry }, ...cur.activity] },
      };
    });
  };

  const handleAction = (account) => (kind, contactName) => {
    const labels = {
    ...
    pushActivity(account, { type, msg: labels[kind] || `${kind} 액션` });
  };

  const handleLog = (account) => ({ type, msg }) => {
    pushActivity(account, { type, msg });
  };

  const handlePinNote = (account) => (note) => {
    setDetails(prev => {
      const cur = prev[account._key] || baseDetailFor(account);
      return {
        ...prev,
  ...
  const handleAddNote = (account) => (body) => {
    setDetails(prev => {
      const cur = prev[account._key] || baseDetailFor(account);
      return {
        ...prev,
        [account._key]: { ...cur, notes: [{ at: '방금', pinned: false, body }, ...cur.notes] },
      };
    });
  };

  const selectedAcc = filtered.find(a => a._key === selectedKey) || null;
  const editingAccount = editAccountKey ? ACCOUNTS.find(a => a._key === editAccountKey) : null;
  ...
  return (
    <div className="hub-page" style={{ padding: 'var(--section-gap)', display: 'flex', flexDirection: 'column', gap: 'var(--gap)', height: '100%', minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 2, background: 'var(--surface-2)', border: '1px solid var(--line-soft)', borderRadius: 'var(--r-sm)', padding: 2 }}>
          {[{ k: 'cards', l: 'Cards' },{ k: 'list', l: 'List' },{ k: 'detail', l: 'Detail' }].map(t => (
            <button key={t.k} onClick={() => {
              setView(t.k);
              if (t.k === 'detail' && !selectedKey) setSelectedKey(filtered[0]?._key ?? null);
            }} style={{
              padding: '4px 10px', fontSize: 11.5, borderRadius: 4,
              ...
              <div onClick={() => openDetail(a._key)}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                  ...
                  <IconButton
                    ...
                    iconSize={12}
                    tooltip="계정 편집"
                    onClick={(e) => { e.stopPropagation(); setEditAccountKey(a._key); }}
                  />
                </div>
        ...
        <Card pad={false} className="hub-detail-card" style={{ flex: 1, minHeight: 0, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            <DetailPanel
              ...
              onAction={selectedAcc ? handleAction(selectedAcc) : () => {}}
              onLog={selectedAcc ? handleLog(selectedAcc) : () => {}}
              onPinNote={selectedAcc ? handlePinNote(selectedAcc) : () => {}}
              onAddNote={selectedAcc ? handleAddNote(selectedAcc) : () => {}}
              onEdit={() => selectedAcc && setEditAccountKey(selectedAcc._key)}
      ...
      <EditDrawer
        ...
        subtitle={editingAccount ? `${editingAccount.type === 'personal' ? 'Personal' : 'Company'} · 계정 정보 편집` : ''}
        record={editingAccount}
        fields={[
          { key: 'name', label: '이름' },
          { key: 'type', label: '대상', type: 'select', options: [{ value: 'company', label: 'Company' }, { value: 'personal', label: 'Personal' }] },
          { key: 'health', label: '상태', type: 'select', options: [{ value: 'ok', label: 'OK' }, { value: 'warning', label: 'Warning' }, { value: 'risk', label: 'Risk' }] },
          { key: 'value', label: '금액', inputType: 'number', placeholder: '0' },
          { key: 'deals', label: '딜 수', inputType: 'number', placeholder: '0' },
          { key: 'last', label: '마지막 접점' },
          { key: 'lastAt', label: '마지막 접점 시간' },
          { key: 'owner', label: '담당' },
        ]}
        onChange={(key, val) => setAccountEdits(prev => ({ ...prev, [editAccountKey]: { ...prev[editAccountKey], [key]: val } }))}
        onClose={() => setEditAccountKey(null)}
      />
    </div>
  );
}
