...
export function TaskChecklistEditor({ task, onChange, conflict, onUseCurrent, onKeepDraft, disabled = false }) {
  ...
  const error = validateTaskChecklist(items);
  ...
  const add = () => {
    ...
    setItems([...items, { id, title: '', done: false, note: '' }]);
    setFocusId(id);
  };
  ...
  return (
    <section className="hub-task-checklist-editor" aria-label="세부 체크리스트">
      <div className="hub-task-checklist-summary">
        ...
        <p>완료한 세부 항목을 체크하세요. 변경사항은 저장 버튼으로 함께 저장합니다.</p>
          ...
          <div className="hub-task-checklist-item" key={item.id} data-done={item.done}>
            <div className="hub-task-checklist-item-main">
              ...
              <TextField ref={element => { if (element) refs.current.set(item.id, element); else refs.current.delete(item.id); }}
                label={`항목 ${index + 1}`} value={item.title} maxLength={200} placeholder="완료할 세부 단계" disabled={disabled}
                onChange={event => update(item.id, { title: event.target.value })} />
            </div>
            <div className="hub-task-checklist-item-actions">
              <Button size="xs" variant="ghost" disabled={disabled} aria-expanded={notesOpen.get(item.id) ?? Boolean(item.note)}
                onClick={() => setNotesOpen(previous => new Map(previous).set(item.id, !(previous.get(item.id) ?? Boolean(item.note))))}>세부 메모</Button>
              ...
              <IconButton icon="x" tooltip={`${index + 1}번째 항목 삭제`} size={28} disabled={disabled} onClick={() => { setLastRemoved({ item, index }); setItems(items.filter(row => row.id !== item.id)); }} />
            </div>
      ...
      <div className="hub-task-checklist-actions">
        <Button icon="plus" variant="outline" size="sm" disabled={disabled || items.length >= TASK_CHECKLIST_LIMIT} onClick={add}>체크리스트 항목 추가</Button>
        {lastRemoved && !items.some(item => item.id === lastRemoved.item.id) && <Button variant="ghost" size="sm" disabled={disabled || items.length >= TASK_CHECKLIST_LIMIT} onClick={() => {
          const next = [...items]; next.splice(Math.min(lastRemoved.index, next.length), 0, lastRemoved.item); setItems(next); setLastRemoved(null);
        }}>삭제 되돌리기</Button>}
