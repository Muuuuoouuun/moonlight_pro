...
function coachPrompt(review, today) {
  const parts = ['[오늘의 하루 회고]'];
  if (review.energy !== null) parts.push(`에너지: ${review.energy}/5 (${ENERGY_LABELS[review.energy - 1]})`);
  // Top 3 실행 결과(2026-09-23 결과 월시동 1단계) — 같은 날짜의 오늘 3개일 때만 싣는다.
  if (today && today.date === review.reviewDate && Array.isArray(today.focusTasks) && today.focusTasks.length) {
    parts.push('오늘 집중 Top 3 실행 결과:');
    today.focusTasks.forEach((task) => parts.push(`- ${task.title}: ${task.done ? '완료' : '미완료'}`));
  }
...
export function DailyReviewCoach({ review, today = null, disabled, onAppendNote }) {
  ...
  async function generate() {
    setState('loading');
    try {
      const result = await requestPersonaChat({ personaId: 'council', mode: 'advice', draft: coachPrompt(review, today) });
