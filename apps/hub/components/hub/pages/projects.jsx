...
export function Projects({ workspace }) {
  ...
  // 고칠 방법이 아예 없었다. existing 플래그로 persistTask가 PATCH 경로를 타게 한다.
  ...
  // 소프트 삭제 — deleted_at을 찍어 원장에서 감춘다(migration 0021). 하위 할 일은 project_id를
  ...
  const persistTask = React.useCallback(async () => {
    ...
    const existing = Boolean(taskDraft.existing);
    try {
      ...
      await loadLedger();
      setOrderResult({ tone: 'ok', label: existing ? '할 일 업데이트됨' : '할 일 저장됨' });
      return { ok: true, status: data.status };
    } catch (error) {
      setOrderResult({ tone: 'err', label: error instanceof Error ? error.message : String(error) });
      return { ok: false, status: 'error' };
  ...
  const deleteTask = React.useCallback(async () => {
    if (!taskDraft?.existing || !taskDraft.id) return { ok: false, status: 'error' };
    try {
      const response = await fetch('/api/hub/tasks', {
        method: 'DELETE',
        headers: { 'content-type': 'application/json' },
  ...
  return (
    <div className="hub-workspace-shell" style={{ display: 'grid', gridTemplateColumns: sidebarHidden ? '1fr' : '240px 1fr', height: '100%', overflow: 'hidden' }}>
      ...
      <EditDrawer
        title={taskDraft?.existing ? (taskDraft.title || '할 일 편집') : '할 일 만들기'}
        subtitle={taskDraft?.existing ? '프로젝트 실행 항목 편집' : '프로젝트 실행 항목'}
        record={taskDraft}
        fields={[
        ...
        onSave={persistTask}
        onDelete={taskDraft?.existing ? deleteTask : undefined}
