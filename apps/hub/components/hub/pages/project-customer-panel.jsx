...
function CustomerPicker({ project, onSaved, onCancel, onPendingChange, onFinished }) {
  ...
  const searchInput = React.useRef(null), errorMessage = React.useRef(null);
  React.useEffect(() => { if (error && !saving) errorMessage.current?.focus({ preventScroll: true }); }, [error, saving]);
  React.useEffect(() => { searchInput.current?.focus({ preventScroll: true }); }, []);
  ...
  async function search(event) {
    ...
    try {
      const response = await fetch(`/api/hub/projects/customer?q=${encodeURIComponent(query)}`, { signal: AbortSignal.timeout(20000) });
      const result = await response.json();
      if (request.current !== ticket) return;
      if (!response.ok || result.status !== 'live') throw Error(result.message || '고객 저장소 연결을 확인해 주세요.');
      setRows(result.contexts || []);
  ...
  async function save() {
    ...
    finally { writing.current = false; setSaving(false); onPendingChange(false); }
    if (saved) onFinished();
  }
  return <section className="project-focus-stack" aria-label="고객 연결 선택">
    ...
    <form className="project-customer-search" onSubmit={search}>
      <TextField ref={searchInput} label="고객·기관명 검색" value={query} maxLength={100} disabled={busy || saving} onChange={(event) => setQuery(event.target.value)}
    ...
    {busy ? <Skeleton lines={3} height={14} label="고객 찾는 중" /> : rows && <div className="project-customer-results" aria-label="찾은 고객">
      {rows.length ? rows.map(row => <Button key={`${row.type}:${row.id}`} className="hub-row" style={{ height: 'auto', whiteSpace: 'normal', padding: '12px', justifyContent: 'space-between' }} active={selected?.type === row.type && selected?.id === row.id} aria-pressed={selected?.type === row.type && selected?.id === row.id} disabled={saving} onClick={() => setSelected(row)}>
        <span className="project-customer-result-copy"><span>{row.label}</span><span className="project-focus-muted">{row.type === 'lead' ? '리드' : '계약 고객'}{row.description ? ` · ${row.description}` : ''}</span>{row.recordId && <span className="project-focus-muted mono">기록 ID {row.recordId}</span>}</span>
    ...
    {selected !== undefined && <p role="status">{selected ? `${selected.label}${selected.description ? ` · ${selected.description}` : ''} 연결` : '이 프로젝트에서 고객 연결만 해제합니다.'}</p>}
    {saving && <p role="status" data-customer-pending="true" tabIndex={-1}>고객 연결을 저장하고 있어요. 완료 후 이동할 수 있어요.</p>}
    {error && <p ref={errorMessage} role="alert" tabIndex={-1} className="project-focus-error">{error}</p>}
...
export function ProjectCustomerPanel({ project, onBack, onSaved, onMemo, onOpenMemo, onPendingChange }) {
  ...
  const root = React.useRef(null), savingRef = React.useRef(false);
  const [saving, setSaving] = React.useState(false);
  ...
  const goBack = () => { if (!savingRef.current) onBack(); };
  React.useEffect(() => {
    const frame = requestAnimationFrame(() => root.current?.querySelector(selecting ? 'input' : 'button')?.focus({ preventScroll: true }));
    return () => cancelAnimationFrame(frame);
  }, []);
  return <div ref={root} className="project-focus-stack" onKeyDown={(event) => {
    if (event.key === 'Escape') {
      event.stopPropagation();
      if (!event.nativeEvent.isComposing && event.keyCode !== 229) goBack();
    }
  }}>
    <div className="project-focus-actions"><Button variant="ghost" icon="chevronL" disabled={saving} onClick={goBack}>프로젝트로</Button>
      {!selecting && <Button variant="ghost" onClick={() => setSelecting(true)}>연결 변경</Button>}</div>
    {selecting ? <CustomerPicker project={project} onCancel={() => ref ? setSelecting(false) : goBack()} onSaved={onSaved} onPendingChange={updatePending} onFinished={goBack} />
