          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {items.map((task, index) => (
              <div
                ...
                style={{
                  // 44px 터치 타깃 플로어(§11)는 지키되, 그 위로는 밀도를 따라간다 — 56px
                  // 고정값은 relaxed(44px)보다도 커서 compact에서 전혀 줄지 않았다.
                  minHeight: 'max(44px, calc(var(--row-h) + 12px))',
                  display: 'flex',
                  alignItems: 'center',
                  gap: 10,
                  padding: 'calc(var(--pad-y) - 4px) 10px calc(var(--pad-y) - 4px) var(--pad-x)',
