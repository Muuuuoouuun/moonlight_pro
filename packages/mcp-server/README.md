...
MCP server that exposes Moonlight Hub actions as tools, so an MCP-compatible Claude
client can read and act on the same live data the Hub UI shows — without a bespoke
webhook per feature.
...
2. Env vars (same names as `apps/hub/.env.example`):
   ...
   - `COM_MOON_HUB_WRITE_SECRET` — required for write tools (`decide_work_order`,
     `create_calendar_event`, `create_campaign`). Must match the Hub's own
...
Add to your MCP config (`~/.claude/mcp.json` or project-level `.mcp.json`):
...
- Revenue/Projects/Content-Queue read tools — same pattern, just not wired up in this
  first pass. Adding one is: a `hubGet` call + a `registerTool` block in `tools.js`.
