...
export function registerMoonlightTools(server,{profile='all',mode='auto'}={}){
  ...
  registerLegacyTools({registerTool(name,definition,handler){
    const read=!name.startsWith('create_')&&name!=='decide_work_order';
    const schema=read?{...definition.inputSchema,detail:z.enum(['summary','rows','full']).optional(),limit:z.number().int().min(1).max(100).optional(),offset:z.number().int().min(0).optional()}:definition.inputSchema;
    collector.registerTool(name,{...definition,inputSchema:schema,outputSchema:z.object({status:z.string().optional()}).passthrough(),annotations:{readOnlyHint:read,destructiveHint:false,idempotentHint:read,openWorldHint:!read}},async(args={})=>{
