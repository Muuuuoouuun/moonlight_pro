...
export interface OfficeWorkflowAnswer {summary:string;artifact:{kind:'text'|'markdown'|'code';body:string};evidence:{sourceRefId:string;explanation:string}[];uncertainties:string[];dissent:string[];council?:{perspectives:{ownerId:OfficeId;judgment:string;tradeoff:string}[];recommendation:string};nextStep:null|{kind:'create_task';label:string;fields:{title:string;description?:string;nextAction?:string;dueAt?:string;dealId?:string;priority?:'low'|'medium'|'high'|'critical'}};sourceCheck?:OfficeSourceCheck}
...
export type OfficeWorkflowResult = OfficeWorkflowMeta & ((OfficeWorkflowAnswer & {status:'generated';resultRevision:1;context:{asOf:string;contextHash:string;missing:string[]};generation:OfficeWorkflowGeneration;discussion?:OfficeDiscussion})|{status:'preview'|'error';error:string;failure?:import('./office.js').OfficeFailure});
...
export const OFFICE_SOURCE_CHECKS:readonly OfficeSourceCheck[];
